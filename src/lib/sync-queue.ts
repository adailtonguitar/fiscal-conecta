 ...
 * Replaces the basic offline-queue.ts with a production-grade sync system.
 */
...
/** Enqueue an item for sync. Lower priority number = synced first. */
...
/** Get all pending items sorted by priority (lower first). */
...
/** Update item status after sync attempt. */
...
/** Mark a failed item for retry if under max retries, else mark as failed. */
...
/** Clear synced and failed items older than `olderThanMs`. */
...
// ===== ENTITY CACHE (for offline product catalog, etc.) =====
