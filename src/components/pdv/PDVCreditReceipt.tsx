...
export function PDVCreditReceipt({ data, onClose }: PDVCreditReceiptProps) {
    ...
    try {
      const receiptContent = `
        ...
        <body>
          ...
          <div class="separator"></div>
          <div class="center line" style="margin-top:24px;">
            <div style="border-top:1px dashed #000; width:80%; margin:0 auto;"></div>
            <div style="margin-top:4px;">Assinatura do Recebedor</div>
          </div>
          <div class="center line" style="margin-top:16px;">Obrigado!</div>
        </body>
        </html>
      `;
  ...
  const handleThermalPrint = () => {
    try {
      ...
      // For now, log the byte array length as confirmation
      ...
      >
        ...
        <div className="px-6 py-4 border-t border-border space-y-3">
          <div className="flex justify-between text-sm">
            ...
            <span className="font-mono text-foreground">{formatCurrency(data.previousBalance)}</span>
          </div>
          <div className="flex justify-between items-center">
            <span className="text-sm text-muted-foreground">Valor recebido</span>
            <span className="text-lg font-bold font-mono text-primary">{formatCurrency(data.amount)}</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Saldo remanescente</span>
            <span className="font-mono text-foreground">{formatCurrency(data.newBalance)}</span>
          </div>
