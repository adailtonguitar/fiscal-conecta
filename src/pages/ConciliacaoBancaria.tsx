...
import {
  Upload, CheckCircle2, XCircle, Link2, Unlink, Search,
  FileUp, ArrowRightLeft, DollarSign, AlertCircle,
} from "lucide-react";
...
export default function ConciliacaoBancaria() {
  ...
  // Fetch unreconciled financial entries for matching
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (dateIdx === -1 || descIdx === -1 || amountIdx === -1) {
        toast.error("Colunas obrigatórias não encontradas. Use: Data, Descrição, Valor");
        return;
      }
  ...
  // Match transaction with financial entry
  const matchMutation = useMutation({
    mutationFn: async ({ txId, entryId }: { txId: string; entryId: string }) => {
      const { error } = await supabase
        .from("bank_transactions")
        .update({ reconciled: true, financial_entry_id: entryId })
        .eq("id", txId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  const unmatchMutation = useMutation({
    mutationFn: async (txId: string) => {
      const { error } = await supabase
        .from("bank_transactions")
        .update({ reconciled: false, financial_entry_id: null })
        .eq("id", txId);
      if (error) throw error;
    },
    onSuccess: () => {
  ...
  // Auto-match: tries to find financial entries with same amount and close date
  const autoMatch = async () => {
    ...
    for (const tx of pending) {
      const candidates = entries.filter((e) => {
        const amountMatch = Math.abs(Number(e.amount) - Number(tx.amount)) < 0.01;
        const typeMatch =
          (tx.type === "credit" && e.type === "receber") ||
          (tx.type === "debit" && e.type === "pagar");
      ...
      if (candidates.length === 1) {
        const { error } = await supabase
          .from("bank_transactions")
          .update({ reconciled: true, financial_entry_id: candidates[0].id })
          .eq("id", tx.id);
        if (!error) matched++;
      }
    }
  ...
  // Suggested matches for selected transaction
  const suggestions = useMemo(() => {
    ...
    return entries
      ...
      .sort((a, b) => {
        const diffA = Math.abs(Number(a.amount) - Number(selectedTx.amount));
        const diffB = Math.abs(Number(b.amount) - Number(selectedTx.amount));
        return diffA - diffB;
      })
      .slice(0, 10);
  }, [selectedTx, entries]);
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-muted-foreground text-sm">Importe extratos e concilie com lançamentos financeiros</p>
        </div>
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={autoMatch}>
            <ArrowRightLeft className="w-4 h-4 mr-1" /> Auto-Conciliar
          </Button>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Saldo Extrato</p>
              <p className="text-lg font-bold">{fmt(stats.totalCredit - stats.totalDebit)}</p>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          {filtered.length === 0 ? (
            <p className="text-muted-foreground text-sm text-center py-12">
              {transactions.length === 0
                ? "Nenhuma transação importada. Use o botão 'Importar CSV' para começar."
                  ...
                  {filtered.map((tx: any) => (
                    <TableRow key={tx.id}>
                      ...
                      <TableCell className={`text-right font-medium ${tx.type === "credit" ? "text-green-600" : "text-red-500"}`}>
                        {tx.type === "debit" ? "- " : ""}{fmt(Number(tx.amount))}
                      </TableCell>
                      <TableCell>
                      ...
                      <TableCell>
                        {tx.reconciled ? (
                          <Badge variant="outline" className="text-green-600 border-green-300 bg-green-50">
                            <CheckCircle2 className="w-3 h-3 mr-1" /> Conciliada
                          </Badge>
                        ) : (
                      ...
                      <TableCell className="text-right">
                        {tx.reconciled ? (
                          ...
                          >
                            <Unlink className="w-4 h-4 mr-1" /> Desfazer
                          </Button>
                        ) : (
                          <Button
                            variant="ghost"
                            size="sm"
                            onClick={() => { setSelectedTx(tx); setMatchOpen(true); }}
                          >
                            <Link2 className="w-4 h-4 mr-1" /> Conciliar
                          </Button>
                        )}
                      </TableCell>
                    </TableRow>
      ...
      <Dialog open={importOpen} onOpenChange={setImportOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Importar Extrato Bancário (CSV)</DialogTitle>
          </DialogHeader>
          <div className="space-y-4">
            <p className="text-sm text-muted-foreground">
              O arquivo CSV deve conter as colunas: <strong>Data</strong>, <strong>Descrição</strong> e <strong>Valor</strong>.
              Valores negativos serão tratados como débito. Separador: vírgula ou ponto-e-vírgula.
            </p>
            <Input
              ref={fileInputRef}
              type="file"
      ...
      {/* Match Dialog */}
      <Dialog open={matchOpen} onOpenChange={(o) => { setMatchOpen(o); if (!o) setSelectedTx(null); }}>
          ...
          {selectedTx && (
            <div className="space-y-4">
              <div className="p-3 rounded-lg bg-muted/50 text-sm">
                <p><strong>Data:</strong> {format(new Date(selectedTx.transaction_date + "T12:00:00"), "dd/MM/yyyy")}</p>
                <p><strong>Descrição:</strong> {selectedTx.description}</p>
                <p><strong>Valor:</strong> {fmt(Number(selectedTx.amount))} ({selectedTx.type === "credit" ? "Crédito" : "Débito"})</p>
              </div>
              <p className="text-sm font-medium">Lançamentos sugeridos (ordenados por proximidade de valor):</p>
              {suggestions.length === 0 ? (
                <p className="text-sm text-muted-foreground">Nenhum lançamento compatível encontrado.</p>
              ) : (
                        ...
                        return (
                          <TableRow key={entry.id}>
                            ...
                            <TableCell className="text-right">
                              ...
                              {diff < 0.01 && (
                                <Badge variant="secondary" className="ml-2 text-xs">Exato</Badge>
                              )}
                            </TableCell>
                            <TableCell>
                              <Button
                                size="sm"
                                onClick={() => matchMutation.mutate({ txId: selectedTx.id, entryId: entry.id })}
                              >
