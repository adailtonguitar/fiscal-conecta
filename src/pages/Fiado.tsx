...
export default function Fiado() {
  ...
  const totalDebt = useMemo(() => {
    return clients.reduce((sum, c) => sum + Number(c.credit_balance || 0), 0);
  }, [clients]);

  const debtorCount = useMemo(() => {
    return clients.filter((c) => Number(c.credit_balance || 0) > 0).length;
  }, [clients]);

  const selectedClient = clients.find((c) => c.id === selectedClientId);
  const clientBalance = Number(selectedClient?.credit_balance || 0);

  // Entries for selected client (receivables)
  ...
  const executePayment = useCallback(async (payAmount: number, registerInCash: boolean, session: any) => {
    ...
    try {
      ...
      // Mark matching financial entries as paid (up to received amount)
  ...
  return (
    <div className="flex flex-col min-h-full">
      ...
      <div className="px-6 py-5 border-b border-border">
        ...
        <p className="text-sm text-muted-foreground mt-1">Gerencie vendas a prazo e receba pagamentos dos clientes</p>
      </div>
        ...
        <Card>
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold font-mono text-destructive">{formatCurrency(totalDebt)}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          ...
          <CardContent className="flex items-center gap-4 p-4">
            ...
            <div>
              ...
              <p className="text-lg font-bold text-foreground">
                {entries.filter((e) => e.type === "receber" && e.status === "pendente").length}
              </p>
            </div>
          </CardContent>
        </Card>
              ...
              <div className="p-4 border-b border-border flex items-center justify-between">
                <div>
                  ...
                  <div className="flex gap-3 mt-1 text-xs text-muted-foreground">
                    <span>{selectedClient.cpf_cnpj || "Sem documento"}</span>
                    {selectedClient.phone && <span>• {selectedClient.phone}</span>}
                  </div>
                </div>
                <div className="text-right">
                  <p className="text-xs text-muted-foreground">Saldo devedor</p>
                  <p className="text-xl font-bold font-mono text-destructive">{formatCurrency(clientBalance)}</p>
                  {Number(selectedClient.credit_limit || 0) > 0 && (
                    <p className="text-xs text-muted-foreground">
                      Limite: {formatCurrency(Number(selectedClient.credit_limit))}
                    </p>
                  )}
                </div>
              </div>
                    ...
                    {clientEntries.map((entry) => (
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Button
                            size="sm"
                            onClick={() => handleReceivePayment(Number(entry.amount))}
