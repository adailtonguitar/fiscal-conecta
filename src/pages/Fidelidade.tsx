  ...
  return (
    <div className="grid gap-6 md:grid-cols-2">
        ...
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">Habilitar acúmulo e resgate de pontos</p>
            </div>
          ...
          <div className="space-y-2">
            ...
            <Input type="number" value={form.points_per_real} onChange={(e) => setForm({ ...form, points_per_real: Number(e.target.value) })} min={0} step={0.1} />
            <p className="text-xs text-muted-foreground">Ex: 1 = cada R$1 gasto = 1 ponto</p>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Valor do ponto no resgate (R$)</label>
            <Input type="number" value={form.redemption_value} onChange={(e) => setForm({ ...form, redemption_value: Number(e.target.value) })} min={0} step={0.01} />
            <p className="text-xs text-muted-foreground">Ex: 0.01 = 100 pontos = R$1,00 de desconto</p>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Mínimo para resgate (pontos)</label>
            <Input type="number" value={form.min_redemption_points} onChange={(e) => setForm({ ...form, min_redemption_points: Number(e.target.value) })} min={1} />
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          <div className="space-y-2">
            <label className="text-sm font-medium">Bônus de boas-vindas (pontos)</label>
            <Input type="number" value={form.welcome_bonus} onChange={(e) => setForm({ ...form, welcome_bonus: Number(e.target.value) })} min={0} />
            <p className="text-xs text-muted-foreground">Pontos ganhos ao fazer a primeira compra</p>
          </div>

          <div className="space-y-2">
            <label className="text-sm font-medium">Multiplicador de aniversário</label>
            <Input type="number" value={form.birthday_multiplier} onChange={(e) => setForm({ ...form, birthday_multiplier: Number(e.target.value) })} min={1} step={0.5} />
            <p className="text-xs text-muted-foreground">Pontos multiplicados no mês de aniversário</p>
          </div>
          ...
          <div className="p-4 rounded-lg bg-primary/5 border border-primary/10 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Compra de R$ 100,00 = <strong>{Math.floor(100 * form.points_per_real)} pontos</strong>
            </p>
            <p className="text-xs text-muted-foreground">
              {form.min_redemption_points} pontos = <strong>R$ {(form.min_redemption_points * form.redemption_value).toFixed(2)} de desconto</strong>
            </p>
          </div>
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-2xl font-bold">{topClients.reduce((s, c) => s + c.loyalty_points, 0).toLocaleString("pt-BR")}</p>
            <p className="text-xs text-muted-foreground">Total de pontos ativos</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="pt-6 text-center">
            ...
            <p className="text-2xl font-bold">
              R$ {(topClients.reduce((s, c) => s + c.loyalty_points, 0) * (config?.redemption_value || 0.01)).toFixed(2)}
            </p>
            <p className="text-xs text-muted-foreground">Valor em pontos (potencial)</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card>
        ...
        <CardContent>
          {topClients.length === 0 ? (
            <p className="text-sm text-muted-foreground text-center py-8">Nenhum cliente com pontos ainda. Os pontos serão acumulados automaticamente nas vendas.</p>
          ) : (
            <div className="space-y-2">
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-muted-foreground">{client.phone || client.cpf_cnpj || "—"}</p>
                    </div>
                    <Badge variant="outline" className={tier.color}>
                      {tier.icon} {tier.name}
                    </Badge>
                    <div className="text-right">
                      <p className="text-sm font-bold text-primary">{client.loyalty_points.toLocaleString("pt-BR")}</p>
...
function TransactionsTab() {
  ...
  const clientMap = new Map(topClients.map((c) => [c.id, c.name]));
            ...
            {recentTransactions.map((tx) => {
              ...
              return (
                <div key={tx.id} className="flex items-center gap-3 p-3 rounded-lg bg-muted/50 border border-border">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center bg-muted ${cfg.color}`}>
                    <Icon className="w-4 h-4" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium">{clientMap.get(tx.client_id) || "Cliente"}</p>
                    <p className="text-xs text-muted-foreground truncate">{tx.description}</p>
                  </div>
                  <Badge variant="outline" className={cfg.color}>{cfg.label}</Badge>
                  <div className="text-right">
                    <p className={`text-sm font-bold ${tx.points > 0 ? "text-success" : "text-destructive"}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            Fidelize seus clientes com sistema de pontos e recompensas
          </p>
        </div>
      ...
      <Tabs defaultValue="ranking" className="space-y-4">
        <TabsList>
          ...
          <TabsTrigger value="transactions" className="flex items-center gap-1.5">
            <History className="w-3.5 h-3.5" /> Transações
          </TabsTrigger>
