...
import { ChevronLeft, ChevronRight, DollarSign, Users, TrendingUp, Download, Percent } from "lucide-react";
...
export default function Comissoes() {
  ...
  // Fetch sales (fiscal_documents) for the period
  ...
  const commissionData = useMemo(() => {
    const grouped: Record<string, { employee: typeof employees[0]; totalSales: number; salesCount: number; commission: number }> = {};

    sales.forEach((sale) => {
      const emp = sale.issued_by ? employeeByUserId[sale.issued_by] : null;
      if (!emp) return;
      const rate = Number((emp as any).commission_rate || 0);
      if (!grouped[emp.id]) {
        grouped[emp.id] = { employee: emp, totalSales: 0, salesCount: 0, commission: 0 };
      }
  ...
  const chartData = commissionData.map((c) => ({
    name: c.employee.name.split(" ")[0],
    vendas: Number(c.totalSales.toFixed(2)),
    comissao: Number(c.commission.toFixed(2)),
  }));

  const exportCSV = () => {
    const header = "Vendedor;Cargo;Taxa %;Vendas;Qtd Vendas;Comissão\n";
    const rows = commissionData
      .map((c) =>
        `${c.employee.name};${c.employee.role || ""};${(c.employee as any).commission_rate || 0}%;${c.totalSales.toFixed(2)};${c.salesCount};${c.commission.toFixed(2)}`
      )
      .join("\n");
    ...
    a.href = url;
    a.download = `comissoes_${start}_${end}.csv`;
    a.click();
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{fmt(totalCommission)}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-lg font-bold">{sellersCount}</p>
            </div>
          </CardContent>
        </Card>
      </div>
      ...
      {chartData.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle className="text-base">Vendas vs Comissão por Vendedor</CardTitle>
          </CardHeader>
          <CardContent>
        ...
        <CardContent>
          {commissionData.length === 0 ? (
            <p className="text-muted-foreground text-sm text-center py-8">
              Nenhuma venda com vendedor identificado neste período.
              Certifique-se de que os funcionários tenham o campo "user_id" vinculado e taxa de comissão definida.
            </p>
          ) : (
                  ...
                  {commissionData.map((c) => (
                    <TableRow key={c.employee.id}>
                      ...
                      <TableCell className="text-right">
                        <Badge variant="secondary">{(c.employee as any).commission_rate || 0}%</Badge>
                      </TableCell>
