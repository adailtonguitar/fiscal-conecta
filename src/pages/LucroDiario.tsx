...
export default function LucroDiario() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase.rpc("get_daily_profit_report", {
    ...
    <div className="p-6 space-y-6 max-w-3xl mx-auto">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button variant="outline" size="icon" onClick={() => setDate(d => addDays(d, 1))} disabled={isToday}>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="py-12 text-center">
            <p className="text-muted-foreground">Nenhuma venda registrada neste dia.</p>
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <Card>
            <CardContent className="pt-4 pb-4">
              <div className="flex items-center justify-between mb-2">
                ...
                <Badge variant={Number(data.margin) >= 15 ? "default" : Number(data.margin) >= 0 ? "secondary" : "destructive"}>
                  {Number(data.margin).toFixed(1)}%
                </Badge>
              </div>
              <div className="h-2 bg-muted rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full ${Number(data.margin) >= 15 ? "bg-success" : Number(data.margin) >= 5 ? "bg-warning" : "bg-destructive"}`}
                  style={{ width: `${Math.max(0, Math.min(100, Number(data.margin)))}%` }}
                />
              </div>
            </CardContent>
          </Card>
            ...
            <Card>
              ...
              <CardContent>
                <div className="space-y-2">
                  {Object.entries(data.by_payment as Record<string, number>).map(([method, value]) => (
                    <div key={method} className="flex items-center justify-between py-1.5 border-b border-border last:border-0">
                      ...
                      <span className="text-sm font-mono font-medium">{formatCurrency(Number(value))}</span>
                    </div>
                  ))}
                </div>
              </CardContent>
