        ...
        {costNum > 0 && marginNum > 0 && (
          ...
          >
            <div className="bg-primary/10 rounded-lg p-3 text-center">
              ...
              <p className="text-lg font-bold text-primary">{formatCurrency(idealPrice)}</p>
            </div>
            ...
            <div className="bg-destructive/10 rounded-lg p-3 text-center">
              <p className="text-xs text-muted-foreground">Impostos (~10%)</p>
              <p className="text-lg font-bold text-destructive">{formatCurrency(estimatedTax)}</p>
            </div>
          </motion.div>
        )}
      </CardContent>
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-2xl font-bold text-foreground">Painel de Lucro Real</h1>
          <p className="text-sm text-muted-foreground mt-1">Análise inteligente de rentabilidade</p>
        </div>
        <div className="flex gap-2">
          <button
            onClick={() => setPeriod("current")}
            className={`px-3 py-1.5 text-xs rounded-lg font-medium transition-colors ${period === "current" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
            Mês Atual
          </button>
          <button
            onClick={() => setPeriod("last")}
            className={`px-3 py-1.5 text-xs rounded-lg font-medium transition-colors ${period === "last" ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
          >
            Mês Anterior
          </button>
        </div>
          ...
          <Card>
            <CardContent className="pt-6">
              <div className="flex items-center justify-between mb-2">
                <span className="text-sm font-medium">Margem Líquida Real</span>
                <span className={`text-lg font-bold ${data.netMargin >= 0 ? "text-success" : "text-destructive"}`}>
                  {data.netMargin.toFixed(1)}%
                </span>
              </div>
              <div className="h-3 bg-muted rounded-full overflow-hidden">
                <div
                  className={`h-full rounded-full transition-all ${data.netMargin >= 15 ? "bg-success" : data.netMargin >= 5 ? "bg-warning" : "bg-destructive"}`}
                  style={{ width: `${Math.max(0, Math.min(100, data.netMargin))}%` }}
                />
              </div>
            </CardContent>
          </Card>
            ...
            <Card className="border-destructive/30">
              <CardHeader>
                <CardTitle className="text-base flex items-center gap-2 text-destructive">
                  ...
                  Produtos Vendendo com Prejuízo ({data.sellingAtLoss.length})
                </CardTitle>
              </CardHeader>
              <CardContent>
                <div className="space-y-2">
                  {data.sellingAtLoss.map((p) => (
                    <div key={p.id} className="flex items-center justify-between py-2 border-b border-border last:border-0">
                      ...
                      <Badge variant="destructive">
                        {formatCurrency(p.totalProfit)} prejuízo
                      </Badge>
                    </div>
                  ))}
                </div>
          ...
          <Card>
            ...
            <CardContent>
              {data.products.length === 0 ? (
                <p className="text-sm text-muted-foreground py-4 text-center">Nenhuma venda no período selecionado</p>
              ) : (
                      ...
                      {data.products.slice(0, 20).map((p, i) => (
                        <tr key={p.id} className="border-b border-border last:border-0 hover:bg-muted/50">
                          ...
                          <td className="py-2 px-2 text-right">
                            <Badge variant={p.margin >= 15 ? "default" : p.margin >= 0 ? "secondary" : "destructive"}>
                              {p.margin.toFixed(1)}%
                            </Badge>
                          </td>
                          <td className={`py-2 px-2 text-right font-mono font-semibold ${p.totalProfit >= 0 ? "text-success" : "text-destructive"}`}>
