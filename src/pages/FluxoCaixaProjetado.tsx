...
import { format, parseISO, addDays, startOfDay, eachDayOfInterval, addMonths, isBefore, isAfter } from "date-fns";
import { ptBR } from "date-fns/locale";
import { motion } from "framer-motion";
import {
  ChevronLeft, ChevronRight, TrendingUp, TrendingDown,
  Download, Calendar, AlertTriangle, Eye, EyeOff,
} from "lucide-react";
...
export default function FluxoCaixaProjetado() {
  ...
  // Fetch pending + paid entries for the projection window
  const { data: entries = [], isLoading } = useFinancialEntries({
    startDate,
    endDate,
  });

  // Fetch current cash balance from last session
  const { data: currentBalance = 0 } = useQuery({
    queryKey: ["cash_balance_projected", companyId],
    queryFn: async () => {
      if (!companyId) return 0;
      // Get the latest closed session's closing balance, or open session's running total
  ...
  const { chartData, dailyDetails, totals, alerts } = useMemo(() => {
    ...
    const dailyMap: Record<string, { entradas: number; saidas: number; items: typeof pendingEntries }> = {};
    days.forEach(d => {
      dailyMap[format(d, "yyyy-MM-dd")] = { entradas: 0, saidas: 0, items: [] };
    });
    ...
    pendingEntries.forEach(entry => {
      ...
      if (dailyMap[key]) {
        ...
        if (entry.type === "receber") {
          ...
          dailyMap[key].saidas += amount;
        }
        dailyMap[key].items.push(entry);
      }
    });

    const chartPoints: Array<{ date: string; label: string; saldo: number; entradas: number; saidas: number }> = [];
    const details: Array<{ date: string; label: string; entradas: number; saidas: number; saldo: number; items: typeof pendingEntries }> = [];
    ...
    days.forEach(d => {
      ...
      if (runningBalance < 0) {
        negativeAlerts.push(format(d, "dd/MM/yyyy"));
      }
    });
    ...
    return {
      ...
      totals: { entradas: totalEntradas, saidas: totalSaidas, saldoFinal: runningBalance },
  ...
  const handleExportCSV = () => {
    ...
    a.href = url;
    a.download = `fluxo-caixa-projetado-${range}d.csv`;
    a.click();
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-popover border border-border rounded-lg p-3 shadow-lg text-sm">
        ...
        <p className={cn("font-bold mt-1", data.saldo >= 0 ? "text-primary" : "text-destructive")}>
              ...
              {isLoading ? (
                ...
                dailyDetails.map((day) => (
                  <tr key={day.date} className={cn("border-b border-border last:border-0 hover:bg-muted/50", day.saldo < 0 && "bg-destructive/5")}>
                      ...
                      <div className="space-y-0.5">
                        {day.items.map(item => (
                          <div key={item.id} className="flex items-center gap-2 text-xs">
                            <Badge variant={item.type === "receber" ? "default" : "destructive"} className="text-[10px] px-1.5 py-0">
                              {item.type === "receber" ? "E" : "S"}
                            </Badge>
                    ...
                    <td className={cn("px-5 py-2.5 text-right font-mono font-semibold", day.saldo >= 0 ? "text-primary" : "text-destructive")}>
                      {formatCurrency(day.saldo)}
                    </td>
                  </tr>
                ))
