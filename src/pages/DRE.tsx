...
// DRE structure categories mapping
const RECEITA_CATEGORIES = ["venda", "servico", "comissao"];
const DEDUCAO_CATEGORIES: string[] = []; // impostos sobre vendas will be from tax data
const CUSTO_CATEGORIES = ["fornecedor"];
const DESPESA_OPERACIONAL_CATEGORIES = ["aluguel", "energia", "agua", "internet", "salario", "manutencao"];
const DESPESA_FISCAL_CATEGORIES = ["impostos"];
const OUTRAS_CATEGORIES = ["reembolso", "outros"];
...
export default function DRE() {
  ...
  // Fetch financial entries for the period
  const { data: entries = [], isLoading: loadingEntries } = useFinancialEntries({
    startDate,
    endDate,
  });

  // Fetch sales (fiscal_documents) for the period to get revenue from sales
  ...
  const dreLines = useMemo((): DRELine[] => {
    // Only consider paid entries for realized DRE
    const paidEntries = entries.filter(e => e.status === "pago");
    
    const sumByCategories = (type: "receber" | "pagar", categories: string[]) =>
      paidEntries
        .filter(e => e.type === type && categories.includes(e.category))
        .reduce((s, e) => s + Number(e.paid_amount || e.amount), 0);

    const sumByType = (type: "receber" | "pagar") =>
      paidEntries
        .filter(e => e.type === type)
        .reduce((s, e) => s + Number(e.paid_amount || e.amount), 0);

    // 1. RECEITA BRUTA (from fiscal documents / sales)
    const receitaVendas = salesDocs.reduce((s, d) => s + Number(d.total_value || 0), 0);
    ...
    // 2. DEDUÇÕES (impostos sobre vendas - from financial entries categorized as "impostos" type "pagar")
    const impostosSobreVendas = sumByCategories("pagar", ["impostos"]);
    const receitaLiquida = receitaBruta - impostosSobreVendas;

    // 3. CUSTOS (CMV - Custo da Mercadoria Vendida)
  ...
  const handleExportCSV = () => {
    const csv = [
      ...
      ...dreLines
        ...
        .map(l => `${l.indent ? "  " : ""}${l.label};${l.value.toFixed(2)}`),
    ].join("\n");
  ...
  return (
    <div className="p-4 sm:p-6 space-y-4 sm:space-y-6 max-w-4xl mx-auto">
      ...
      <motion.div
        ...
        className="bg-card rounded-xl card-shadow border border-border overflow-hidden print:shadow-none"
      >
              ...
              {isLoading ? (
                ...
                dreLines.map((line, i) => {
                  if (line.separator) {
                    return (
                      <tr key={i}>
                        <td colSpan={2} className="h-1 bg-border/50" />
                      </tr>
                    );
                  }
                  return (
                    <tr
                      ...
                      className={cn(
                        ...
                        line.highlight && "bg-muted/40"
                      )}
                    >
                      <td
                        className={cn(
                          "px-5 py-2.5 text-foreground",
                          line.bold && "font-semibold",
                          !line.bold && "text-muted-foreground"
                        )}
                      ...
                      <td
                        className={cn(
                          ...
                          line.bold && "font-semibold",
                          line.value > 0 && "text-primary",
                          line.value < 0 && "text-destructive",
                          line.value === 0 && "text-muted-foreground"
                        )}
                      >
                        {line.value === 0 && !line.bold ? "—" : formatCurrency(Math.abs(line.value))}
                        {line.value < 0 && line.bold && (
                          <span className="text-xs ml-1 text-destructive">▼</span>
                        )}
                      </td>
                    </tr>
                  );
